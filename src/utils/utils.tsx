...
export const saveTokenWithExpiration = (token: string, expiresIn: number) => {
    ...
    saveToken(token);
    sessionStorage.setItem(`${TOKEN_STORAGE_KEY}_expiration`, expirationTime.toString());
};

// Check if the token is expired
export const isTokenExpired = (): boolean => {
    const expirationTime = sessionStorage.getItem(`${TOKEN_STORAGE_KEY}_expiration`);
    if (!expirationTime){
